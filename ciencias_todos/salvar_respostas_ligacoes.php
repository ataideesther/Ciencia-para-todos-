...
try {
    ...
    try {
        // Primeiro, deletar todas as respostas anteriores do aluno para as questões atuais
        $stmt = $conn->prepare("
            DELETE FROM resposta 
            WHERE id_aluno = :id_aluno 
            AND id_questao IN (" . implode(',', array_column($data['respostas'], 'id_questao')) . ")
        ");
        ...
        // Preparar query para inserir novas respostas
        $stmt = $conn->prepare("
            INSERT INTO resposta 
            (id_aluno, id_questao, resposta_dada) 
            VALUES 
            (:id_aluno, :id_questao, :resposta_dada)
        ");
        ...
        $stmt = $conn->prepare("
            ...
            AND r.id_questao IN (" . implode(',', array_column($data['respostas'], 'id_questao')) . ")
        ");
