document.addEventListener('DOMContentLoaded', function () {
    ...
    function showResults() {
        ...
        const respostasParaEnviar = questions.map((question, index) => ({
            ...
            resposta_dada: userAnswers[index]
        }));

        // Calcular pontuação
        const pontuacao = userAnswers.reduce((acc, resposta, index) => {
            return acc + (resposta === questions[index].correctAnswer ? 1 : 0);
        }, 0);
        ...
        .then(data => {
            if (data.success) {
                ...
                window.location.href = `resultado_quiz.php?pontuacao=${pontuacao}&total=${questions.length}`;
            } else {
