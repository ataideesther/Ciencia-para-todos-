...
try {
    ...
    // Buscar o ID da trilha de citologia
    $stmt = $conn->prepare("SELECT id_trilha FROM trilhas WHERE nome_trilha LIKE '%Citologia%'");
    $stmt->execute();
    $trilha = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$trilha) {
        throw new Exception('Trilha de Citologia não encontrada');
    }
